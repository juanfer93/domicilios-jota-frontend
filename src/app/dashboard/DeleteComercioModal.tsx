...
type DeleteComercioModalProps = {
  comercioToDelete: ComercioItem | null;
  onCancel: () => void;
  onConfirm: () => Promise<void> | void;
};

export function DeleteComercioModal({
  comercioToDelete,
  onCancel,
  onConfirm,
}: DeleteComercioModalProps) {
  if (!comercioToDelete) return null;
    ...
    <div className="fixed inset-0 z-50 flex items-center justify-center bg-black/50">
      <div className="bg-[#FFF9E8] rounded-3xl shadow-2xl w-full max-w-sm mx-4 p-4 sm:p-6">
        ...
        <p className="mt-2 text-xs sm:text-sm text-[#174A8B]/80">
          Se eliminará{" "}
          <span className="font-semibold">{comercioToDelete.nombre}</span>{" "}
          ({comercioToDelete.direccion}). Esta acción no se puede deshacer.
        </p>
